...
app.addEventListener('click', e => {
  ...
  if (el.dataset.round !== undefined) { selected = Number(el.dataset.round); setup = false; navigate('scorecard'); }
...
app.addEventListener('submit',e=> { if(e.target.id!=='course-setup') return; e.preventDefault(); const f=new FormData(e.target); const pars=Array.from({length:18},(_,i)=>Number(f.get('par'+i))); const indexes=Array.from({length:18},(_,i)=>Number(f.get('si'+i))); if(new Set(indexes).size!==18 || indexes.some(x=>!Number.isInteger(x)||x<1||x>18) || pars.some(x=>!Number.isInteger(x)||x<3||x>6)) return toast('Use valid pars and every stroke index from 1 to 18 exactly once.'); Object.assign(data.rounds[selected],{pars,indexes,verified:true}); setup=false; save(); render(); toast('Course details saved. Scores recalculated.'); });
