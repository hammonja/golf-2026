...
const PLAYER_BIOS = [
  'A seasoned contender with years in the field and the current lead in the prestigious Peter Cup. The man to catch.',
  'A former rugby international who brings serious power off the tee. Currently runner-up in the Peter Cup and chasing the top spot.',
  'A former pro with a single-figure handicap, returning to the field. One to watch when the competition heats up!',
  'The youngest of the fourball, with a smoking driver and a game rising rapidly through the ranks. Watch this space.'
];
...
function render() { ({ dashboard, scorecard, players, rules, gallery: () => Media.gallery() }[page] || dashboard)(); }
...
window.addEventListener('hashchange',()=> { const target=location.hash.slice(1); if(['dashboard','scorecard','players','rules','gallery'].includes(target)) {page=target;render();} });
if(['dashboard','scorecard','players','rules','gallery'].includes(location.hash.slice(1))) page=location.hash.slice(1);
updateRankMovement();
render();
if (typeof fetch === 'function') Live.start();
if (typeof Media !== 'undefined') Media.start();
