...
function mobileHoleEditor() {
  ...
  return `<section class="mobile-hole-editor" aria-label="Hole-by-hole scoring">
    <div class="hole-toolbar"><button data-mobile-prev ${hole === 0 ? 'disabled' : ''} aria-label="Previous hole">←</button><div><span class="eyebrow">${COURSES[selected]} · ${hole < 9 ? 'FRONT' : 'BACK'} NINE</span><h2>Hole ${hole+1} <span>/ 18</span></h2><span class="hole-facts">${teeDistance(selected,hole) ? teeDistance(selected,hole)+' &middot; ' : ''}Par ${r.pars[hole]} <b>·</b> Stroke index ${r.indexes[hole]}${r.verified ? '' : ' · provisional'}</span></div><button data-mobile-next ${hole === 17 ? 'disabled' : ''} aria-label="Next hole">→</button></div>
...
function decorateMobile() {
  ...
  if (page === 'dashboard') {
    ...
    const cards = `<div class="mobile-standings">${rows.map(row=>`<article class="card mobile-standing"><div class="standing-top"><span class="standing-rank">${started ? rows.findIndex(x=>x.points===row.points)+1 : '—'}${movementMarkup(row.p)}</span>${avatar(row.p)}<div class="standing-name"><strong>${row.name}</strong><small>HCP ${data.handicaps[row.p]} · ${row.holes}/54 holes</small></div><div class="standing-points">${row.points}<small>PTS</small></div></div><div class="standing-stats"><span>Gross<strong>${row.holes ? row.gross : '—'}</strong></span><span>Net<strong>${row.holes ? row.net : '—'}</strong></span><span>Stableford<strong>${row.holes ? row.stable : '—'}</strong></span><span>Pin bonus<strong>+${row.bonus}</strong></span></div><div class="standing-rounds">${row.round.map((r,i)=>`<span>R${i+1} <b>${r.ranking}</b></span>`).join('')}<span>placing points</span></div><div class="mobile-earnings"><span>Prize money won</span><div>${earningsMarkup(row)}</div></div></article>`).join('')}</div>`;
