...
async function main() {
  ...
  assert(!viewer.document.querySelector('.report-jump'),'An incomplete round never shows a report link');
  snapshot.state.rounds[2].scores.forEach(row=>row.fill(4));snapshot.sequence++;push();await tick();
  const reportLink='[data-report-round="2"]';
  const panel=viewer.document.querySelector('.competition-grid').children[2];
  assert.equal(panel.lastElementChild,viewer.document.querySelector(reportLink),'Report link belongs at the bottom of its competition panel');
  assert(!viewer.document.querySelector('.round-reports'),'Reports are not printed below the panels');
  assert(!viewer.document.querySelector('#round-report-dialog'),'Report stays closed until requested');
  click(viewer,reportLink);
  const dialog=viewer.document.querySelector('#round-report-dialog');
  assert(dialog.open,'Viewers can open the report popup');
  assert.equal(dialog.querySelector('h2').textContent,'<script>unsafe</script>');
  assert(!dialog.querySelector('script'),'AI text is escaped');
  assert(viewer.document.body.classList.contains('report-open'));
  snapshot.sequence++;push();await tick();
  assert.equal(viewer.document.querySelector('#round-report-dialog'),dialog,'Live updates preserve the open popup');
  snapshot.summaries[2]={status:'pending'};snapshot.sequence++;push();await tick();
  assert(dialog.open);
  assert(!dialog.textContent.includes('James led'),'A stale report disappears while its replacement is generated');
  assert(dialog.textContent.includes('Writing the round report'));
  assert(viewer.document.querySelector('.competition-grid [role="status"]'));
  snapshot.summaries[2]={status:'failed'};snapshot.sequence++;push();await tick();
  assert(dialog.querySelector('[data-action="summary-retry-2"]').hidden,'Viewer cannot retry generation');
  snapshot.summaries[2]=report;snapshot.sequence++;push();await tick();
  assert(dialog.textContent.includes('James led'),'A replacement report appears in the open popup');
  click(viewer,'[data-report-close]');
  assert(!viewer.document.querySelector('#round-report-dialog'));
  assert(!viewer.document.body.classList.contains('report-open'));
  assert.equal(viewer.document.activeElement,viewer.document.querySelector(reportLink),'Closing returns focus to the current link');
  click(viewer,reportLink);
  viewer.document.querySelector('#round-report-dialog').dispatchEvent(new viewer.w.MouseEvent('click',{bubbles:true,clientX:-1,clientY:-1}));
  assert(!viewer.document.querySelector('#round-report-dialog'),'Tapping outside closes the popup');
  click(viewer,reportLink);
  snapshot.summaries[2]=report;snapshot.state.rounds[2].scores[0][17]=null;snapshot.sequence++;push();await tick();
  assert(!viewer.document.querySelector('.report-jump'),'Clearing a score removes the report link');
  assert(!viewer.document.querySelector('#round-report-dialog'),'Clearing a score also closes the popup');
  assert(!viewer.document.body.classList.contains('report-open'));
